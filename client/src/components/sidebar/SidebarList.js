    ...
    const setSideLists = () =>
    {
        ...
        try
        {
            if(index!=-1 && todoLists.length > 0)
            {
                ...
                newList.push(
                <SidebarEntry
                    _activeid={props._activeid} handleSetActive={props.handleSetActive} activeid={props.activeid}
        ...
        } catch(err) { return props.todolists.filter(checkLevel).map(todo => (
            <SidebarEntry
                _activeid={props._activeid}
                handleSetActive={props.handleSetActive} activeid={props.activeid}
        ...
        return props.todolists.filter(checkLevel).map(todo => (
                <SidebarEntry
                    _activeid={props._activeid}
                    handleSetActive={props.handleSetActive} activeid={props.activeid}
                    id={todo.id} key={todo.id} name={todo.name} _id={todo._id}
